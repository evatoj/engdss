...
public class LedgerEventConsumer {
    ...
    public void onSaqueIniciado(Map<String, Object> payload) {
        try {
            ...
            boolean aprovado = ledgerCommandService.debitPending(cmd);
            if (aprovado) {
                log.info("DEBIT_PENDING aplicado. correlationId={}", cmd.correlationId());
            } else {
                log.warn("DEBIT_DENIED — saldo insuficiente. correlationId={} accountId={}", cmd.correlationId(), cmd.accountId());
            }
        } catch (Exception e) {
